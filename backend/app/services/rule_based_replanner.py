...
class RuleBasedReplanner:
    ...
    def propose(self, replan_context: ReplanRecord) -> ReplanRecord:
        ...
        filtered = [
            ...
            if self._matches_strategy(candidate, old_poi, risk_type, strategy)
        ]
        if not filtered:
            return asdict(
                self._empty_proposal(
    ...
    def _empty_proposal(
        ...
        return ReplanProposal(
            ...
            strategy=self._text(replan_context.get("strategy"), "CONTINUE"),
            reason=reason,
            old_card_id=old_card_id or self._text(replan_context.get("current_card_id"), "") or None,
    ...
    def _queue_level(self, queue: ReplanRecord) -> str:
        level = self._text(queue.get("queue_level"), "").lower()
